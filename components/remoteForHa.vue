...
import OneItem from "./oneItemForHa.vue";
...
export default {
  ...
  data() {
    return {
      skuIdToTypeMap: Object.keys(eventCode).reduce((prev, cur) => {
        prev[eventCode[cur]] = cur;
        return prev;
      }, {}),
  ...
  computed: {
    ...
    addItems() {
      let fields = ["phone", "password", "must", "max", "remark", "showPrice"];
  ...
  methods: {
    ...
    async confirmEdit() {
      ...
      if (form.showPrice) {
        form.price = form.showPrice.split(",");
      } else {
        form.price = [];
      }
      delete form.showPrice;
      let data = {
    ...
    async add() {
      ...
      if (this.form.showPrice) {
        data.price = this.form.showPrice.split(",");
      }
    ...
    async getConfig(isFirst) {
      ...
      try {
        ...
        data.forEach((one) => {
          ...
          one.cmd = cmd;
          one.showPrice = one.price ? one.price.join(",") : "";
          one.loading = false;
