<template>
    <div class="input-search">
        <div class="placeholder text" v-if="!value" @click="showDialog">{{ placeholder }}</div>
            ...
            <div class="content safe-bottom">
                <div class="top">
                    <div class="left">
                        ...
                        <div class="after-wrap" @click="clear">
                            <image class="after" v-if="showValue" src="../../static/clear.png" alt="" />
                        </div>
                    </div>
                ...
                <scroll-view class="real-content" scroll-y @scrolltolower="getMore">
                    <div class="list" v-if="list.length">
                        <div class="item" v-for="(item, index) in list" :key="index" @click="setValue(item)"
    ...
    methods: {
        ...
        async search(isClear) {
            ...
            if (this.platform === 'xingqiu') {
                let { data: { searchData } } = await request({
                    noHandleCode: true,
                    url: "https://m.piaoxingqiu.com/cyy_gatewayapi/home/pub/v3/show_list/search?src=WEB&channelId=&terminalSrc=WEB&offset=0&length=10&keyword=" + encodeURIComponent(this.showValue),
                })
                ...
                let test = await request({
                    ...
                    url: "https://search.damai.cn/searchajax.html?cty=&ctl=&sctl=&tsg=0&st=&et=&order=0&pageSize=30&currPage=1&keyword=" + encodeURIComponent(this.showValue),
                })
                console.log(123, test)
                let { pageData: { resultData } } = test
                res = {
                    records: resultData,
